...
abc2svg.jianpu={k_tb:["Cb","Gb","Db","Ab","Eb","Bb","F","C","G","D","A","E","B","F#","C#"],cde2fcg:new Int8Array([0,2,4,-1,1,3,5]),cgd2cde:new Int8Array([0,-4,-1,-5,-2,-6,-3,0,-4,-1,-5,-2,-6,-3,0]),acc2:new Int8Array([-2,-1,3,1,2]),acc_tb:["\ue264","\ue260",,"\ue262","\ue263","\ue261"],do_pscom:function(of,p){switch(p.match(/\w+/)[0]){case'staves':case'score':p=p.replace(/\(|\)/g,'')
break}
...
return}
function set_head(){var tsfirst=abc.get_tsfirst(),p_v=voice_tb[0],mt=p_v.meter.a_meter[0],sk=p_v.key,s={type:C.BLOCK,subtype:"text",dur:0,v:0,p_v:p_v,st:0,seqst:true,text:(sk.k_mode+1)+"="+
(abc2svg.jianpu.k_tb[sk.k_sf+7+
abc2svg.jianpu.cde2fcg[sk.k_mode]])},s2=voice_tb[0].sym
if(mt)
...
n=2
while(--n>=0){s2={type:C.REST,v:s.v,p_v:s.p_v,st:s.st,dur:C.BLEN/4,dur_orig:C.BLEN/4,stem:0,multi:0,nhd:0,notes:[{pit:s.notes[0].pit,jn:8}],xmx:0,noplay:true,time:s.time+C.BLEN/4,prev:s,next:s.next}
s.next=s2
if(s2.next)
s2.next.prev=s2
...
break}}
s=s2}}
function set_sym(p_v){var s,s2,note,pit,nn,p,a,m,i,sf=p_v.key.k_sf
...
if(s.dur>=C.BLEN/2)
slice(s)
if(s.a_dd){for(i=0;i<s.a_dd.length;i++){if(s.a_dd[i].glyph=="stc"){abc.deco_cnv(["gstc"],s)
s.a_dd[i]=s.a_dd.pop()}}}}}
set_head()
...
if(s.nflags>0){if(s.beam_st||s.type==C.REST){nl=s.nflags
s2=s
...
if(note.acc&&s.wl<14)
s.wl=14}
break}},set_hooks:function(abc){abc.do_pscom=abc2svg.jianpu.do_pscom.bind(abc,abc.do_pscom)
