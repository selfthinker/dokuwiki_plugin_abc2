...
abc2svg.grid3={block_gen:function(of,s){if(s.subtype!="grid"){of(s)
return}
...
this.cfmt().csdef=txt
if(opt.indexOf("noprint")<0){if(this.parse.state>=2){s=this.new_block(type)
...
function add_cs(ss,ch){var s={type:C.REST,fname:ss.fname,istart:ss.istart,iend:ss.iend,v:p_vc.v,p_v:p_vc,time:ntim,st:0,dur:0,dur_orig:0,invis:true,seqst:true,nhd:0,notes:[{pit:18,dur:0}]}
if(ch!='.'&&ch!='-'){abc.set_a_gch(s,[{type:'g',text:ch,otext:ch,istart:ss.istart,iend:ss.iend,font:abc.get_font("gchord")}])}
