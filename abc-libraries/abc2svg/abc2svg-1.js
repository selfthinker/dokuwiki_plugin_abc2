...
for(var k in obj)
if(obj.hasOwnProperty(k)){if(lvl&&typeof obj[k]!="number")
...
up=s.stem<0
if(up){y=s.ymx|0}else if(dd.name[0]=='w'){de.inv=true
y=s.ymn}else{y=s.ymn-dd.h}
if(y>-6&&y<24){if(up)
y+=3;y=(((y+6)/6)|0)*6-6}
if(up)
s.ymx=y+dd.h
...
if(w<10){x-=10-w
w=10}}
dd=de.dd;y=y_get(st,up,x-dd.wl-5,w)
...
return}
var yc,up,inv,s=de.s,dd=de.dd,x=de.x,w=dd.wl+dd.wr,stafft=staff_tb[s.st].topbar+2,staffb=staff_tb[s.st].botbar-2
if(s.nhd)
...
inv=true
if(dd.name!="invertedfermata"&&(up>0||(up<0&&s.multi>=0))){yc=y_get(s.st,true,s.x-dd.wl,w)+2+dd.hd
if(yc<stafft)
yc=stafft;y_set(s.st,true,s.x-dd.wl,w,yc+dd.h);s.ymx=yc+dd.h}else{yc=y_get(s.st,false,s.x-dd.wl,w)-3+dd.hd
if(yc>staffb)
yc=staffb;yc-=dd.h;y_set(s.st,false,s.x-dd.wl,w,yc-dd.hd)
if(dd.name=="fermata")
inv=true;s.ymn=yc}
break}
...
function deco_def(nm){var a,dd,dd2,name2,c,i,elts,str,hd,text=decos[nm]
...
return}
if(c_func>10&&(c_func<32||c_func>43)){error(1,null,"%%deco: bad C function index '$1'",c_func)
return}
if(c_func==5)
c_func=3
if(c_func==7)
...
if(!dd){dd={name:nm}
dd_tb[nm]=dd}
dd.func=dd.name.indexOf("head-")==0?9:c_func;dd.glyph=a[2];dd.h=Number(h)
dd.hd=Number(hd)
...
prev.notes[j].dur*=2
continue
case 35:if(s.type!=C.NOTE){error(1,s,errs.must_note,dd.name)
continue}
...
continue
dcn="sacc"+s.notes[0].acc.toString()
dd=dd_tb[dcn]
if(!dd){dd=deco_def(dcn)
...
x+=note.shhd*stv_g.scale}else if(f_staff[dd.func]&&!cfmt.dynalign&&((de.up&&st>0)||(!de.up&&st<nstaff))){if(de.up)
ym=ymid[--st]
else
...
create_deco(s)
if(s.notes){for(var m=0;m<s.notes.length;m++){if(s.notes[m].a_dcn)
create_dh(s,m)}}}
...
break}}
if(j==n_de){de2={s:s,st:de.st,dd:dd2,ix:a_de.length-1,x:realwidth-6,y:s.y,lden:true,defl:{noen:true}}
if(de2.x<s.x+10)
de2.x=s.x+10
if(de.m!=undefined)
de2.m=de.m;a_de.push(de2)}
de2.start=de;de2.defl.nost=de.defl.nost
if(i>0&&a_de[i-1].s.time==de.s.time&&a_de[i-1].dd.name.slice(0,dd.name.length-1)==dd.name.slice(0,dd.name.length-1))
de2.prev=a_de[i-1]}
...
for(s=tsfirst;s;s=s.ts_next){if(s.type!=C.PART||s.invis)
continue
...
if(top+dy<ymin+h+ht)
dy=ymin+h+ht-top
for(s=some_part;s;s=s.ts_next){if(s.type!=C.PART||s.invis)
continue
...
var acc_nd={}
function draw_acc(x,y,a){if(typeof a!="number"){var c,n=a[0],d=a[1]
c=n+'_'+d
a=acc_nd[c]
...
s2.notes[0].pit=shift/3+18;self.draw_hl(s2)
last_shift=shift;draw_acc(x,staffb+shift,acc.acc)
x+=5.5}}}
...
s2=s
while(!s2.seqst)
s2=s2.ts_prev;s2=s2.ts_prev;x=(x+s2.x+s2.wr)/2
if(s.a_dd)
...
if(s.rep_nb>2&&s.v==cur_sy.top_voice&&cfmt.measrepnb>0&&!(s.rep_nb%cfmt.measrepnb))
...
default:set_hl(p_staff,j+1,x,-7,7)
break}
if(i==9){y-=6;j--}
break
case 7:y+=6;j++
...
default:set_hl(p_staff,j,x,-7,7)
break}}
...
if(s.p_v.ckey.k_bagpipe||!cfmt.graceslurs||slur||s.tie_s||!s.next||s.next.type!=C.NOTE)
return
last=g
...
if(old_color!=false)
set_color(old_color)}
function draw_note(s,fl){var s2,i,m,y,staffb,slen,c,nflags,x,y,note,y_tb=new Array(s.nhd+1)
if(s.dots)
setdoty(s,y_tb)
note=s.notes[s.stem<0?s.nhd:0];x=x_head(s,note)
...
note.sls=nsls}}}
s=s.next}}
function draw_tuplet(s1){var s2,s3,g,upstaff,nb_only,some_slur,x1,x2,y1,y2,xm,ym,a,s0,yy,yx,dy,a,dir,r,tp=s1.tp.shift()
if(!s1.tp.length)
delete s1.tp
upstaff=s1.st
set_dscale(s1.st)
...
if(s3==s2)
break
y_set(upstaff,true,s3.x,s3.next.x-s3.x,yy)}else if(s3==s2){break}}}else{if(s1.stem<0)
x1-=2
...
function draw_note_ties(not1,job){var m,x1,x2,s,y,h,time,not2=not1.tie_n,p=job==2?not1.pit:not2.pit,dir=(not1.tie_ty&0x07)==C.SL_ABOVE?1:-1,s1=not1.s,st=s1.st,s2=not2.s,x2=s2.x,sh=not1.shhd
for(m=0;m<s1.nhd;m++)
if(s1.notes[m]==not1)
break
...
if(!p_st.top){p_st.top=new Float32Array(YSTEP)
...
function set_staff(){var s,i,st,prev_staff,v,y,staffsep,dy,maxsep,mbot,val,p_voice,p_staff,sy=cur_sy
for(v=0;v<voice_tb.length;v++){p_voice=voice_tb[v]
if(p_voice.scale!=1)
...
Abc.prototype.draw_symbols=function(p_voice){var bm={},s,g,x,y,st;for(s=p_voice.sym;s;s=s.next){if(s.invis){switch(s.type){case C.KEY:p_voice.ckey=s
default:continue
case C.NOTE:break}}
st=s.st
...
x2=sx(hl[0])
output+='m'+
(x2-xp+hl[1]-dx2).toFixed(1)+' 0h'+(-hl[1]+hl[2]).toFixed(1)
xp=x2
dx2=hl[2]}
output+='"/>\n'}}
for(st=0;st<=nstaff;st++){p_st=staff_tb[st]
...
return
xxxfont="u"+n+"font"}
font=cfmt[xxxfont];if(!font){font={pad:0}}else{font={name:font.name,size:font.size,box:font.box,pad:font.pad}}
...
if(a){font.pad=a[1]?Number(a[1]):0
p=p.replace(a[0],a[2])}
a=p.match(/\s+class=(.*?)(\s|$)/)
...
break
case"beamslope":case"breaklimit":case"lineskipfac":case"maxshrink":case"pagescale":case"parskipfac":case"scale":case"slurheight":case"stemheight":case"stretchlast":case"tieheight":f=parseFloat(param)
...
(typeof document=="undefined"?cfmt.fullsvg:"")+"{"+style_font(font)+"}")
if(font.src)
...
blk_flush()
cfmt=sav.cfmt;info=sav.info;char_tb=sav.char_tb;glovar=sav.glovar;maps=sav.maps;mac=sav.mac;maci=sav.maci;parse.tune_v_opts=null;parse.scores=null;init_tune()
...
function lktsym(s,next){s.ts_next=next
if(next){s.ts_prev=next.ts_prev
if(s.ts_prev)
s.ts_prev.ts_next=s;next.ts_prev=s}else{error(2,s,"Bad linkage")
s.ts_prev=null}
s.seqst=!s.ts_prev||s.time!=s.ts_prev.time||(w_tb[s.ts_prev.type]!=w_tb[s.type]&&w_tb[s.ts_prev.type]!=0)
if(!next)
return
next.seqst=next.time!=s.time||(w_tb[s.type]!=w_tb[next.type]&&w_tb[s.type]!=0)
if(next.seqst){self.set_width(next)
...
l=len-((C.BLEN/16/8)<<i)
space=space_tb[i]
if(l){if(l<0){space=space_tb[0]*len/(C.BLEN/16/8)}else{if(i>=9)
i=8
...
if(s==s_et)
break}}
function _bar(s){return{type:C.BAR,bar_type:"|",fname:s.fname,istart:s.istart,iend:s.iend,v:s.v,p_v:s.p_v,st:s.st,dur:0,nhd:0,notes:[{pit:s.notes?s.notes[0].pit:22}],prev:s}}
...
return b}
function set_allsymwidth(){var maxx,val,st,s_chs,tim,stup,s=tsfirst,s2=s,xa=0,xl=[],wr=[]
maxx=xa
tim=s.time
while(1){do{if(s.a_gch&&!s_chs)
...
if(!s2)
return}
p_voice=s.p_v;p_voice.last_sym=s.prev;p_voice.time=s.time;new_s=sym_add(p_voice,C.CUSTOS);new_s.next=s;s.prev=new_s;lktsym(new_s,s);new_s.shrink=s.shrink
if(new_s.shrink<8+4)
...
if(!t[3]){t1=t[1]+t[2]
t2='['}else if(!t[1]){t1=t[2][0]=='['?t[2].slice(1):t[2]
...
if(!s1)
return so
for(s=so;;s=s.ts_next){switch(s.type){default:if(w_tb[s.type])
break
case C.KEY:case C.METER:case C.CLEF:continue}
break}
...
for(s2=s;s2;s2=s2.ts_next){switch(s2.type){case C.KEY:if(!cfmt.keywarn||(!s2.k_a_acc&&!s2.k_sf&&!s2.k_old_sf)||s2.k_none||s2.k_play)
continue
for(s1=s.ts_prev;s1;s1=s1.ts_prev){if(s1.type!=C.METER)
break}
case C.METER:if(s2.type==C.METER){if(!cfmt.timewarn)
continue
s1=s.ts_prev}
...
s=bardiv(s)
do_warn(s)
switch(s.ts_prev.type){case C.STAVES:break
...
function get_width(s,next){var shrink,space,w=0,wmx=0,sp_fac=(1-cfmt.maxshrink)
...
wmx+=next.wr
return[w,wmx]}
function set_lines(s,next,lwidth,indent){var first,s2,s3,x,xmin,xmid,xmax,wwidth,shrink,space,ws,nlines,last=next?next.ts_prev:null
ws=get_width(s,next)
if(cfmt.keywarn&&next&&next.type==C.KEY&&!last.dur){ws[0]+=next.wr
ws[1]+=next.wr}
...
next=set_nl(next)
return next||last}
s2=first=s;xmin=s.x-s.shrink-indent;xmax=xmin+lwidth;xmid=xmin+wwidth/nlines;xmin+=wwidth/nlines*cfmt.breaklimit;for(s=s.ts_next;s!=next;s=s.ts_next){if(!s.x)
continue
if(s.type==C.BAR)
s2=s
if(s.x>=xmin)
...
s3=s
break}
if(!s3){var beam=0,bar_time=s2.time;xmax-=8;s=s2;for(;s!=next;s=s.ts_next){if(s.beam_st)
beam++
...
function mexp(s){var s2,s3,s4,next,tim,nbar,nb=s.nmes,dur=s.dur/nb
s.type=C.REST
...
s3=s2=s
while(--nb>0){if(next){s2=clone(next)
delete s2.soln
delete s2.a_gch
delete s2.a_dd
lkvsym(s2,next)}else{s2=_bar(s)
s.next=s2}
s2.time=tim
while(s3.time<tim)
s3=s3.ts_next
...
function init_music_line(){var p_voice,s,s1,s2,s3,last_s,v,st,shr,shrmx,shl,shlp,p_st,top,nv=voice_tb.length
...
break}
unlksym(s)
case C.PART:case C.TEMPO:case C.BLOCK:case C.REMARK:s=s.ts_next
continue}
break}
last_s=tsfirst
...
insert_meter&=~1}
for(v=0;v<nv;v++){p_voice=voice_tb[v]
if(p_voice.sls.length){s={type:C.BAR,fname:last_s.fname,bar_type:"|",dur:0,multi:0,invis:true,sls:p_voice.sls}
new_sym(s,p_voice,last_s)
p_voice.sls=[]}}
for(v=0;v<nv;v++){p_voice=voice_tb[v];s2=p_voice.bar_start;p_voice.bar_start=null
if(last_s&&last_s.v==v){for(s3=last_s;s3;s3=s3.ts_next){if(s3.v!=v)
break
switch(s3.type){case C.PART:case C.TEMPO:continue
case C.BAR:p_voice.last_sym=s3
last_s=s3.ts_next
break}
break}}
if(!s2)
...
if(nflags<=0){s.beam_st=true;s.beam_end=true}else if(s.type==C.NOTE||s.beam_on){s.beam_st=true;start_flag=false}}
if(s.beam_end)
start_flag=true
...
if(beam){s.stem=laststem}else if(s.beam_st&&!s.beam_end){beam=true;n=pu=0
for(g=s;g;g=g.next){if(g.type!=C.NOTE){continue}
if(g.stem||g.multi){s.stem=g.stem||g.multi
break}
n+=g.nhd+1
for(m=0;m<=g.nhd;m++){if(g.notes[m].pit>=mid_p)
pu++}
if(g.beam_end)
break}
if(!s.stem){pu*=2
if(pu>n){s.stem=-1}else if(pu<n){s.stem=1}else{pu=s.notes[s.nhd].pit;pd=s.notes[0].pit
for(g=s.next;g;g=g.next){if(g.type!=C.NOTE){continue}
...
if(g.beam_end)
break}
if(g.beam_end){if(pu+pd<mid_p*2){s.stem=1}else if(pu+pd>mid_p*2){s.stem=-1}else{if(cfmt.bstemdown)
s.stem=-1}}}}
if(!s.stem)
s.stem=laststem}else{n=(s.notes[s.nhd].pit+s.notes[0].pit)/2
if(n==mid_p){for(m=0;m<=s.nhd;m++){if(s.notes[m].pit>=mid_p)
break}
...
s1.dot_low=false
break
case 0:if(s1.notes[i1].acc!=s2.notes[i2].acc&&!s1.notes[i1].acc&&!s2.notes[i2].acc){t=-1
break}
...
var blocks=[]
Abc.prototype.block_gen=function(s){switch(s.subtype){case"leftmargin":case"rightmargin":case"pagescale":case"pagewidth":case"scale":case"staffwidth":blk_flush()
...
if(indent){posx-=indent;insert_meter&=~2}
while(blocks.length)
...
return}
pit[i]=242}else{pit[i]=abc2svg.pab40(note.pit,note.acc)}}
return pit[1]-pit[0]}
...
break
case"octave=":val=parseInt(a.shift())
if(isNaN(val))
syntax(1,errs.bad_val,item)
else
curvoice.octave=val
...
set_ref(s);i=1
if(sf<-1){switch(param[0]){case'H':if(param[1].toLowerCase()!='p'){syntax(1,"Unknown bagpipe-like key")
key_end=true
break}
s.k_bagpipe=param[1];sf=param[1]=='P'?0:2;i++
if(!cfmt.temper)
cfmt.temper=new Float32Array([11.62,12.55,2.37,2.37,3.49,0,2.37,2.37,3.49,4.41,5.53,0,3.49,4.41,5.53,7.35,7.35,4.41,5.53,7.35,7.35,8.19,0,7.35,7.35,8.19,9.39,10.51,0,8.19,9.39,10.51,11.62,12.55,0,10.51,11.62,12.55,2.37,2.37])
break
...
if(nd){d=Number(nd[2])
if(d&&!isNaN(d)&&!(d&(d-1))){n=Number(nd[1])
if(!isNaN(n))
return C.BLEN*n/d}}
...
if(!nd)
return
s.new_beat=nd}else{s.tempo=Number(text)
if(!s.tempo||isNaN(s.tempo)){syntax(1,"Bad tempo value")
return}}}
if(parse.state!=3){if(parse.state==1){info.Q=txt
glovar.tempo=s
return}
goto_tune()}
sym_link(s)
...
d2=d2?Number(a[3])/d2*C.BLEN:0}else{d2=d1}}else if(text=="auto"){d1=d2=-1}
...
function new_bar(dotted){var s2,c,bar_type,line=parse.line,s={type:C.BAR,fname:parse.fname,istart:parse.bol+line.index,dur:0,multi:0}
if(dotted)
s.bar_dotted=true
if(vover&&vover.bar)
get_vover('|')
...
s.norepbra=true
if(s.text){if(bar_type.slice(-1)=='['&&bar_type!='[')
bar_type=bar_type.slice(0,-1)
if(s.text[0]=='1'){if(curvoice.tie_s)
curvoice.tie_s_rep=curvoice.tie_s
if(curvoice.acc_tie)
curvoice.acc_tie_rep=curvoice.acc_tie.slice()
else if(curvoice.acc_tie_rep)
curvoice.acc_tie_rep=null}else{if(curvoice.tie_s_rep){curvoice.tie_s=clone(curvoice.tie_s_rep)
...
if(curvoice.ulen<0)
adjust_dur(s);if((bar_type=="["||bar_type=="|:")&&!curvoice.eoln){s2=curvoice.last_sym
if(s2&&s2.time==curvoice.time){if(s2&&s2.type==C.BAR){if(bar_type=="["&&!s2.text&&(curvoice.st==0||(par_sy.staves[curvoice.st-1].flags&STOP_BAR)||s.norepbra)){if(s.text)
s2.text=s.text
if(s.a_gch)
s2.a_gch=s.a_gch
if(s.norepbra)
...
continue}
break}
if(vids[vid]){syntax(1,"Double voice in %%score")}else{vids[vid]=true
a_vf.push([vid,flags])}
flags=0
if(!e)
break
...
note.acc=map[1].acc
if(!note.acc&&curvoice.acc[pit+19])
note.acc=3
...
s=-s}
return p0+o+(p1-p0)*s}
Abc.prototype.new_note=function(grace,sls){var note,s,in_chord,c,dcn,type,tie_s,acc_tie,i,n,s2,nd,res,num,dur,apit,sl1=[],line=parse.line,a_dcn_sav=a_dcn
function do_ties(s,tie_s){var m,note,mid
for(m=0;m<=s.nhd;m++){note=s.notes[m]
mid=note.midi
if(tie_s.type!=C.GRACE){for(i=0;i<=tie_s.nhd;i++){if(!tie_s.notes[i].tie_ty)
continue
if(tie_s.notes[i].midi==mid){tie_s.notes[i].tie_n=note
note.s=s
tie_s.tie_s=s
break}}}else{for(s2=tie_s.extra;s2;s2=s2.next){if(!s2.notes[0].tie_ty)
continue
if(s2.notes[0].midi==mid){s2.tie_s=s
...
if(curvoice.tie_s){tie_s=curvoice.tie_s
curvoice.tie_s=null}
...
return}
if(s.nmes==1){s.type=C.REST;s.dur_orig=s.dur;s.notes=[{pit:18,dur:s.dur}]}else{glovar.mrest_p=true}
break
...
return}
if(tie_s&&s.type!=C.NOTE){syntax(1,"Bad tie")
tie_s=null}
...
c=line.char()
while(1){switch(c){case'-':var ty=parse_vpos()
...
line.index=j
return b}
function n2n(n){var c=ntb[n]
while(n<0){n+=7;c+=','}
while(n>14){n-=7;c+="'"}
return c}
function expand(m,b){var c,i,r="",n=m.length
for(i=0;i<n;i++){c=m[i]
...
for(;s;s=s.next)
s.invis=s.play=true
curvoice=curv}else{line.buffer=b?expand(m,b):m;parse_seq(true)}
...
break
if(par_sy.voices[curvoice.v]&&par_sy.voices[curvoice.v].range==0)
curvoice.eoln=true
break
...
var strwh=typeof document!="undefined"?(function(){var el=abc2svg.eltxt
if(!el){abc2svg.eltxt=el=document.createElement('text')
el.style.position='absolute';el.style.top='-1000px';el.style.padding='0';document.body.appendChild(el)}
...
return str.wh}
str=str.replace(/<|>|&[^&]*?;|&|  /g,function(c){switch(c){case'<':return"&lt;"
case'>':return"&gt;"
case'&':return"&amp;"
case"  ":return'  '}
return c})
...
return[w,h]}})():function(str){var font=gene.curfont,swfac=font.swfac,h=font.size,w=0,i,j,c,n=str.length
...
cl=font_class(nf)
return'<tspan\n\tclass="'+cl+'">'}
o=str.replace(/<|>|&[^&]*?;|&|  |\$./g,function(c){switch(c){case'<':return"&lt;"
case'>':return"&gt;"
case'&':return"&amp;"
case'  ':return'  '
default:if(c[0]!='$')
...
if(i<0){str=str2svg(text.slice(j))
...
while(text[i+1]=='\n'){vskip(lineskip);i++}
if(i==text.length)
break}else{str=str2svg(text.slice(j,i))
vskip(str.wh[1]*cfmt.lineskipfac+font.pad*2)
xy_str(x,font.pad,str,action)}
j=i+1}
...
if(w>=strlw){str=str2svg(words.slice(k,j).join(' '))
vskip(str.wh[1]*cfmt.lineskipfac)
xy_str(0,0,str,action,strlw)
k=j;w=ww}
w+=cwidf(' ')}
if(w!=0){str=str2svg(words.slice(k).join(' '))
vskip(str.wh[1]*cfmt.lineskipfac)
xy_str(0,0,str)}
vskip(parskip);blk_flush()
if(i<0)
...
function put_wline(p,x){var i=0,j,k
...
function out_tubrn(x,y,dx,dy,up,str){var sw=str.length*10,h=up?-3:3;set_font("tuplet")
...
str.push('<tspan\nclass="'+
font_class(cfmt.musicfont)+'" style="font-size:'+
(gene.curfont.size*1.3).toFixed(1)+'px" dy="-.05em">'+
p+'</tspan>')
j=p.length>1?2:1
w+=j*gene.curfont.swfac
dy='y'}}
...
if(dy)
s.tempo_dy=dy}
function writempo(s,x,y){var bx
set_font("tempo")
...
bh=gene.curfont.size+4;output+='<rect class="stroke" x="'
...
if(style||font_style)
head+='<style>\n.'+
font_class(font)+' text,tspan{fill:currentColor}'+
font_style+style+'\n</style>\n'
if(defs)
head+='<defs>'+defs+'\n</defs>\n'
...
set_ref(s)
parse.last_sym=s;s.prev=curvoice.last_sym
...
function sort_all(){var s,s2,p_voice,v,time,w,wmin,ir,multi,prev,nb,ir2,v2,fl,new_sy,nv=voice_tb.length,vtb=[],vn=[],sy=cur_sy
for(v=0;v<nv;v++)
...
do_cloning()}
s=glovar.tempo
if(s&&staves_found<=0){v=par_sy.top_voice;p_voice=voice_tb[v];if(p_voice.sym&&p_voice.sym.type!=C.TEMPO&&(!p_voice.sym.next||p_voice.sym.next.type!=C.TEMPO)){s=clone(s);s.v=v;s.p_v=p_voice;s.st=p_voice.st;s.time=0;s.prev=p_voice.sym
...
while(1){var sl=p_voice.sls.shift()
if(!sl)
break
s=sl.note.s
...
break}
continue
case C.MREST:if(s.next&&s.next.type!=C.BAR){syntax(1,"Lack of bar after multi-measure rest")
s2=clone(s)
s2.type=C.BAR
s2.bar_type='|'
s2.dur=0
...
continue
switch(s.type){case C.METER:bar_num+=(s.time-bar_tim)/wmeasure
bar_tim=s.time
wmeasure=s.wmeasure
break
case C.BAR:if(s.invis)
...
if(acc&&acc!=3){if(typeof acc=="number"){nn=['__','_','','^','^^'][acc+2]}else{i=acc[0]
if(i>0){nn+='^'}else{nn+='_'
i=-i}
nn+=i+'/'+acc[1]}}
nn+=ntb[(pit+75)%7]
...
goto_tune()
switch(cmd){case"center":set_font("text")
if(parse.state>=2){s=new_block("text");s.text=param
s.opt='c'
s.font=gene.curfont
return}
write_text(param,'c')
...
return
case"maxsysstaffsep":if(parse.state==3){val=get_unit(param)
if(isNaN(val)){syntax(1,errs.bad_val,"%%maxsysstaffsep")
return}
par_sy.voices[curvoice.v].maxsep=val
return}
break
case"multicol":if(parse.state>=2){if(parse.state==2)
goto_tune()
curvoice=voice_tb[0]
...
return
case"sysstaffsep":if(parse.state==3){val=get_unit(param)
if(isNaN(val)){syntax(1,errs.bad_val,"%%sysstaffsep")
return}
par_sy.voices[curvoice.v].sep=val
return}
break
case"text":set_font("text")
if(parse.state>=2){s=new_block(cmd);s.text=param
...
if(val==undefined){syntax(1,errs.bad_transp)
return}}
switch(parse.state){case 0:cfmt.transp=0
case 1:case 2:cfmt.transp=(cfmt.transp||0)+val
return}
...
break
case"ml":if(parse.state>=2){s=new_block(type);s.text=text}else{blk_flush()
if(user.img_out)
user.img_out(text)}
break
case"svg":j=0
...
set_font("text")
if(parse.state>=2){s=new_block(type);s.text=text
s.opt=action
s.font=gene.curfont
break}
...
return
if(voice_tb.length)
voice_adj(true);var maxtime=0,no_sym=true
for(v=0;v<voice_tb.length;v++){p_voice=voice_tb[v]
...
a_vf[i][0]=p_voice;while(1){par_sy.voices[v]={}
...
parse.state=3;if(!voice_tb.length){get_voice("1");curvoice.clef.istart=curvoice.key.istart;curvoice.clef.iend=curvoice.key.iend;curvoice.default=true}else if(!curvoice){curvoice=voice_tb[staves_found<0?0:par_sy.top_voice]}
if(!curvoice.init&&!is_K){set_kv_parm([])
set_transp()}
...
function get_lyrics(text,cont){var s,word,p,i,j,ly,dfnt
if(curvoice.ignore)
return
...
return}
if(word&&s.pos.voc!=C.SL_HIDDEN){if(word.match(/^\$\d/)){gene.curfont=word[1]=='0'?dfnt:get_font("u"+word[1])
word=word.slice(2)}
...
if(j<0){syntax(1,"No end of guitar chord")
return}
if(line.buffer[j-1]!='\\')
break
i=j+1}
...
function csan_add(s){s.a_gch=a_gch;a_gch=null
if(curvoice.vtransp)
gch_transp(s)}
Abc.prototype.gch_build=function(s){var gch,wh,xspc,ix,y_above=0,y_below=0,y_left=0,y_right=0,GCHPRE=.4;for(ix=0;ix<s.a_gch.length;ix++){gch=s.a_gch[ix]
...
case'>':gch.y-=y_right
break}}}
Abc.prototype.draw_gchord=function(i,s,x,y){var y2,an=s.a_gch[i],h=an.font.size,pad=an.font.pad,w=an.text.wh[0]+pad*2
...
function draw_all_chsy(){var s,san1,an,i,y,w,pos=curvoice.pos.gch==C.SL_BELOW?-1:1,n_an=0,minmax=new Array(nstaff+1)
...
if(!an)
continue
if((an.type=='g'&&pos>0)||an.type=='^'){x=s.x+an.x
w=an.text.wh[0]
if(w&&x+w>realwidth)
x=realwidth-w
y=y_get(s.st,1,x,w)
if(an.type=='g'&&y<minmax[s.st].yup)
y=minmax[s.st].yup}else if((an.type=='g'&&pos<=0)||an.type=='_'){continue}else{x=s.x+an.x
y=an.y}
...
continue
an=an[i]
if(!an||!((an.type=='g'&&pos<0)||an.type=='_'))
continue
x=s.x+an.x
w=an.text.wh[0]
if(w&&x+w>realwidth)
...
w=an.text.wh[0]
if(pos>0){y=y_get(s.st,true,s.x,w)
if(y>minmax[s.st].yup)
minmax[s.st].yup=y}else if(pos<0){y=y_get(s.st,false,s.x,w)
...
Abc.prototype.add_style=function(s){style+=s};Abc.prototype.cfmt=function(){return cfmt};Abc.prototype.clone=clone;Abc.prototype.deco_cnv=deco_cnv;Abc.prototype.defs_add=defs_add
...
abc2svg.version="1.20.16-6c1ff8e813";abc2svg.vdate="2021-01-06"
