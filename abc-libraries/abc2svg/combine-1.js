...
abc2svg.combine={comb_v:function(){var C=abc2svg.C,delsym=[]
...
while(1){nhd=s.nhd;s2=s.ts_next;nhd2=s2.nhd
...
delsym.push({s:s2,r:s});this.unlksym(s2)
if(s.in_tuplet||!may_combine.call(this,s))
break}}
function tie_repl(s){var s1=s.tie_s,i=delsym.length
while(--i>=0){if(delsym[i].s==s1){s.tie_s=delsym[i].r
break}}}
var s,s2,g,i,r
...
if(!s.beam_st)
continue
if(s.beam_end){if(may_combine.call(this,s))
do_combine.call(this,s)
continue}
s2=s
...
break
do{s2=s2.next}while(s2.type!=C.NOTE&&s2.type!=C.REST)}}
for(s=this.get_tsfirst();s;s=s.ts_next){if(s.tie_s)
tie_repl(s)}},do_pscom:function(of,text){if(text.slice(0,13)=="voicecombine ")
