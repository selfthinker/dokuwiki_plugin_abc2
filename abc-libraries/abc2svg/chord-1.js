...
r.push(v)}
return r}
function gench(sb,gch){var r,ch,b,m,n,not,a=gch.otext.match(/([A-G])([#♯b♭]?)([^/]*)\/?(.*)/),s={v:vch.v,p_v:vch,type:C.NOTE,time:sb.time,notes:[]}
if(a)
r=abc2svg.letmid[a[1]]
if(r==undefined){s.type=C.REST
ch=[0]
...
s.ts_next.ts_prev=s
vch.last_sym=s}
if(cfmt.chord.names){chnm=Object.create(abc2svg.chordnames)
for(k in cfmt.chord.names){vch=""
for(i=0;i<cfmt.chord.names[k].length;i++){s=cfmt.chord.names[k][i]
vch+=abc2svg.midlet[s%12]
vch+=i==0?"-":(s>=12?"+":" ")}
chnm[k]=[vch]}}else{chnm=abc2svg.chordnames}
vch={v:voice_tb.length,id:"_chord",time:0,sym:{type:C.REST,time:0},instr:cfmt.chord.prog||0,vol:cfmt.chord.vol||.8}
