...
// changed default conf.sfu to point to CDN due to licensing issues
...
conf.gain=0.7;conf.speed=1;(function get_param(){try{if(!localStorage)
return}catch(e){return}
var v=localStorage.getItem("sfu")
if(v)
conf.sfu=v;v=localStorage.getItem("volume")
if(v)
conf.gain=Number(v)})()
...
s.parts=r
s.p_s=[]
while(1){if(!s.ts_next){s.part=first
break}
s=s.ts_next
if(s.type==C.PART){s.part=first
v=s.text[0]
...
i()}}}}
abc2svg.play_next=function(po){function do_tie(s,midi,d){var i,note,C=abc2svg.C,v=s.v,end_time=s.time+s.dur
while(1){s=s.ts_next
if(!s)
return d
switch(s.type){case C.BAR:if(s.rep_p){if(!po.repn){s=s.rep_p
end_time=s.time}}
if(s.rep_s){if(!s.rep_s[po.repv+1])
return d
s=s.rep_s[po.repv+1]
end_time=s.time}
...
po.midi_ctrl(po,s,t)}
po.p_v[s2.v]=true}
function play_cont(po){var d,i,st,m,note,g,s2,t,maxt,C=abc2svg.C,s=po.s_cur
...
if(po.conf.new_speed){d=po.get_time(po)
po.stim=d-(d-po.stim)*po.conf.speed/po.conf.new_speed
...
if(!s.part)
break
case C.PART:if(s.part&&po.i_p!=undefined){s2=s.part.p_s[++po.i_p]
if(s2){po.stim+=(s.ptim-s2.ptim)/po.conf.speed
s=s2
...
break
case C.NOTE:d=s.pdur/po.conf.speed
for(m=0;m<=s.nhd;m++){note=s.notes[m]
if(note.ti2)
continue
po.note_run(po,s,note.midi,t,note.tie_ty?do_tie(s,note.midi,d):d)}
case C.REST:d=s.pdur/po.conf.speed
...
break}
while(1){if(s==po.s_end||!s.ts_next){if(po.onend)
setTimeout(po.onend,(t-po.get_time(po)+d)*1000,po.repv)
po.s_cur=s
return}
s=s.ts_next
if(!s.noplay)
...
break}
po.s_cur=s
po.timouts.push(setTimeout(play_cont,(t-po.get_time(po))*1000
-300,po))}
...
var abcsf2=[]
function Audio5(i_conf){var po,conf=i_conf,empty=function(){},errmsg,ac,gain,instr=[],params=[],rates=[],w_instr=0
var b64d=[]
...
a[i]=s[i]/196608}
function sf2_create(sf2_bin,instr){var i,sid,gen,parm,sampleRate,sample,infos,parser=new sf2.Parser(sf2_bin)
parser.parse()
...
parm={attack:Math.pow(2,(gen.attackVolEnv?gen.attackVolEnv.amount:-12000)/1200),hold:Math.pow(2,(gen.holdVolEnv?gen.holdVolEnv.amount:-12000)/1200),decay:Math.pow(2,(gen.decayVolEnv?gen.decayVolEnv.amount:-12000)/1200)/3,sustain:gen.sustainVolEnv?(gen.sustainVolEnv.amount/1000):0,buffer:ac.createBuffer(1,sample.length,sampleRate)}
...
if(gen.sampleModes&&(gen.sampleModes.amount&1)){parm.loopStart=parser.sampleHeader[sid].startLoop/sampleRate
...
conf.instr_load(instr,function(sf2_bin){sf2_create(sf2_bin,instr)
if(--w_instr==0)
...
s.p_v.vol=s.val/127}
function note_run(po,s,key,t,d){var g,st,instr=s.instr,k=key|0
parm=po.params[instr][k],o=po.ac.createBufferSource(),v=s.p_v.vol==undefined?1:s.p_v.vol
if(!v||!parm)
return
...
init_b64d()
if(!conf.sfu)
conf.sfu="https://cdn.jsdelivr.net/npm/abc2svg@1.20.16/Scc1t2"
if(!conf.instr_load){conf.instr_load=function(instr,done,fail){abc2svg.loadjs(conf.sfu+'/'+instr+'.js',function(){done(b64dcod(abcsf2[instr]))},fail)}}
...
po={conf:conf,onend:conf.onend||empty,onnote:conf.onnote||empty,s_end:i_end,s_cur:i_start,repv:i_lvl||0,tgen:2,get_time:get_time,midi_ctrl:midi_ctrl,note_run:note_run,ac:ac,gain:gain,params:params,rates:rates}
w_instr++
load_res(i_start)
if(--w_instr==0)
...
break}}},play:function(i_start,i_end,i_lvl){po={conf:conf,onend:conf.onend||empty,onnote:conf.onnote||empty,s_end:i_end,s_cur:i_start,repv:i_lvl||0,tgen:2,get_time:get_time,midi_ctrl:midi_ctrl,note_run:note_run,op:op,c_i:[]}
...
var sty=document.createElement("style");sty.innerHTML=".abcr {fill: #d00000; fill-opacity: 0; z-index: 15}";document.head.appendChild(sty)}
